...
parfor i = 1:length(file_list)
    ...
    %% Current Velocity
    image_spectra_surf_MTF = image_spectra_surf_HP .* sqrt(K.^(1.21));% .* abs(W).^(-0.6));
    image_spectra_wave_MTF = image_spectra_wave_HP .* sqrt(K.^(1.21));% .* abs(W).^(-0.6));
...
save("parameter_ver6.mat", 's_Date', 's_Tp_surf', 's_Tp_wave', 's_Pdir_surf', 's_Pdir_wave', 's_SNR_surf', 's_SNR_wave', 's_Signal_surf', 's_Signal_wave', 's_Noise_surf', 's_Noise_wave', 's_Land_energy');
