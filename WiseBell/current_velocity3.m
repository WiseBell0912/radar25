...
for i = 1 : length(file_list)
    ...
    % image specturm
    image_spectrum = abs(image_fft).^2;

    % hpf
    K_th = 2*pi / 10;
    W_th = 2*pi / 20;
    hpf = (abs(W) > W_th & abs(K) < K_th);
    image_spectrum_hpf = image_spectrum .* hpf;

    % weight
    E_th = 0.2 * max(image_spectrum_hpf, [], "all");
    wf = (image_spectrum > E_th & W > 0);
    ...
    bpf = (bpv1 <= W & W <= bpv2);
    image_spectrum_bpf = image_spectrum_hpf .* bpf;

    % snr
    signal = sum(image_spectrum_bpf, "all");
    noise = sum(image_spectrum_hpf, "all") - signal;
    snr = signal / noise;
