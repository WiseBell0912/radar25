...
for i = 1 : 1
    ...
    hold on;
    % yyaxis left
    plot(bb_Date, movmean(bb_Hs, 12), 'Color', [0, 0, 0, 0.8], 'LineStyle', '-', 'LineWidth',0.3);
    plot(rr_Date, movmean(rr_wave_Hs, 12), 'Color', [1, 0, 0, 0.8], 'LineStyle', '-', 'LineWidth',0.3);
    plot(rr_Date, movmean(rr_surf_Hs, 12), 'Color', [0, 0, 1, 0.8], 'LineStyle', '-', 'LineWidth',0.3);
    ylabel("Hs [m]");
    % yyaxis right
...
% wave
y_mean = mean(bb_Hs);
SS_tot = sum((bb_Hs - y_mean).^2);
SS_res = sum((bb_Hs - rr_wave_Hs).^2);
R_squared = 1 - (SS_res / SS_tot);

figure(2);
hold on;
plot(bb_Hs, rr_wave_Hs, '.', 'MarkerSize', 3);
...
y_mean = mean(bb_Hs);
SS_tot = sum((bb_Hs - y_mean).^2);
SS_res = sum((bb_Hs - rr_surf_Hs).^2);
R_squared = 1 - (SS_res / SS_tot);

figure(3);
hold on;
plot(bb_Hs, rr_surf_Hs, '.', 'MarkerSize', 3);
plot(0:0.5:5, 0:0.5:5, '--', 'Color', 'r', 'LineWidth', 1);
plot(0:0.5:5, 1:0.5:6, '--', 'Color', 'k', 'LineWidth', 1);
plot(0:0.5:5, -1:0.5:4, '--', 'Color', 'k', 'LineWidth', 1);
hold off;
