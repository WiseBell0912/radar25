...
parfor i = 1 : nFile
    ...
    % ---------- SNR ---------- %
    % surf
    surf_signal1 = sum(spectrum_surf_power_win_hp(:));
    surf_noise1 = sum(spectrum_surf_power_win(:)) - surf_signal1;
    surf_SNR_Val1 = surf_signal1 / surf_noise1;
    % wave
    wave_signal1 = sum(spectrum_wave_power_win_hp(:));
    wave_noise1 = sum(spectrum_wave_power_win(:)) - wave_signal1;
    ...
    Kx2D = Kx(:, :, 1);
    Ky2D = Ky(:, :, 1);
    dir_deg = mod(mod(rad2deg(atan2(Ky2D, Kx2D)) - 180, 360) .* K(:, :, 1), 180);
    % surf
